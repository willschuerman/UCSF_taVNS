...
cd(fileparts(which('analyze_SART_physio_data.m')));
%% Background 
...
%% Load data
initials = '501';
data = load(['SART_Microstudy/' filesep  'Physiology' filesep 'SMT_' initials '.mat']);
Fs = 10000;
%% get rid of bad data at end of experiment (for tVNS Danielle.mat)
%data.data = data.data(1:40210900,:);
%% plot raw data
for sp = 1:size(data.data,2)
    subplot(size(data.data,2),1,sp)
    plot(data.data(:,sp))
    title(data.labels(sp,:))
end
...
%% Get ANIN channel
stimChannel = find(~cellfun(@isempty, ...
    strfind(cellstr(data.labels),'CBLCFMA')));
%% get onset of stimulation periods
ANIN = data.data(:,stimChannel);
ANIN = ANIN - mean(ANIN);
...
%plot(onsets,ANIN(onsets),'ro')


%% display impedance signal 
clf
plot(t/60,data.data(:,1)) % it's either 1 or 5
axis tight
xlabel('Time (m)')
hold on
ylabel('Ohms')
...
%%
impedance = data.data(:,1);
newfs = 1250;
impedance = downsample(impedance,8);
plot(impedance)
%%
[Pxx,F] = pwelch(impedance,[],[],[],newfs);
semilogy(F,Pxx)
...
title('ECG')
subplot(3,1,3)
plot(data.data(onsets(1)-10000:onsets(3),4))
title('PPG')
...
%% Fucntions
function [data] = simpleNotch(data,fs,notch_freq)
    cfs = (notch_freq:notch_freq:round(fs/2));
    for cf = cfs
%         clc
%         fprintf('Notch filtering %d of %d....\n', cf, nanmax(cfs));
        order = 2;
        Fc = [cf-2 cf+2];
        Fn = fs/2;
        Wn = Fc/Fn;
